...
def greedy_algorithm(df, need):
    total_v = df['Запаси'].sum()
    total_n = df['Умова знижки'].sum()
    buy_table = pd.DataFrame(columns=['Постачальник', 'Ціна купівлі', 'Кількість тканини'])
    if total_n < need:
        if total_v > need:
            check_sale(df)
            print(df)
            while need > 0:
                min_s = int(df['Ціна'].min())
                ...
                if min_c < min_s or min_c == min_s:
                    ...
                    if future_x <= need:
                        buy_table = buy_table.append({'Постачальник':index_c, 'Ціна купівлі': min_c, 'Кількість тканини': future_x},
                                                     ignore_index=True)
                    else:
                        if need >= future_x2:
                            buy_table = buy_table.append({'Постачальник':index_c,'Ціна купівлі': min_c, 'Кількість тканини': need},
                                                         ignore_index=True)
                        else:
                            new_min_index_s = df.loc[index_c, 'Ціна']
                            buy_table = buy_table.append({'Постачальник':index_c,'Ціна купівлі': new_min_index_s, 'Кількість тканини': need},
                                                         ignore_index=True)
                    ...
                    if new_x <= need:
                        buy_table = buy_table.append({'Постачальник':index_s,'Ціна купівлі': min_s, 'Кількість тканини': new_x},
                                                     ignore_index=True)
                    else:
                        buy_table = buy_table.append({'Постачальник':index_s,'Ціна купівлі': min_s, 'Кількість тканини': need},
                                                     ignore_index=True)
                    df.loc[index_s, 'Знижка'] = np.NaN
                    df.loc[index_s, 'Ціна'] = np.NaN
                last_x = buy_table['Кількість тканини'].iloc[-1]
                need -= last_x
            buy_table.index += 1
            return buy_table
        else:
                    ...
                    if min_c < min_s or min_c == min_s:
                        ...
                        if new_x <= need:
                            buy_table = buy_table.append(
                                {'Постачальник': index_s, 'Ціна купівлі': min_s, 'Кількість тканини': new_x},
                                ignore_index=True)
                        else:
                            buy_table = buy_table.append(
                                {'Постачальник': index_s, 'Ціна купівлі': min_s, 'Кількість тканини': need},
                                ignore_index=True)
                        df.loc[index_s, 'Знижка'] = np.NaN
                        df.loc[index_s, 'Ціна'] = np.NaN
                    last_x = buy_table['Кількість тканини'].iloc[-1]
...
def get_z(df):
    df['Закупка'] = None
    for i in range(1, len(df)+1):
        df.loc[i, 'Закупка'] = int(df.loc[i, 'Ціна купівлі']) * (df.loc[i, 'Кількість тканини'])
    df.astype(int)
    print(df)
    total_z = int(df['Закупка'].sum())
    return total_z
...
def create_line(df1, df2):
    ...
    graph.multi_line([[x], [x]], [[y1], [y2]], color=["firebrick", "navy"], alpha=[0.8, 0.3], line_width=4)
    return graph
...
def main():
    ...
    answer = tsh_2(providers, 100)
    print(answer)
    z =answer['Витрата'].sum()
    print(z)
main()
